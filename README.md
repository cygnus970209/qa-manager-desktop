...
- **새 창 요청 처리** — 창이 하나뿐이라 `target=_blank` 링크·`window.open` 은 새 창을 만들지 않는다. 접속 중인 서버와 같은 origin 이면 같은 창에서 이동, 외부 http(s) 는 OS 기본 브라우저, `mailto:` 는 메일 앱으로 넘긴다 (`handle_new_window`)
